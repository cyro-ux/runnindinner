...
async function syncPayment(db, paymentId) {
  ...
  try {
    if (!customerId) {
      ...
      // Bij naam-collision (eerdere klant met dezelfde basis-naam, bv. uit
      // testdata of een eerdere editie) probeert Zoho geen auto-suffix toe
      // te voegen. We vangen de 400-error af en retry'en met een datum-
      // suffix zodat elke runningdinner.app-betaling toch zijn eigen
      // customer-record krijgt — accountant houdt 1-op-1 mapping.
      let created;
      try {
        created = await zoho.createCustomer({ name: rdaName, ...customerOpts });
      } catch (err) {
        if (/bestaat al|already exists/i.test(String(err.message))) {
          const dateSuffix = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
          const fallbackName = `${rdaName} (${dateSuffix})`;
          console.warn(`[zoho-sync] customer "${rdaName}" already exists, retrying as "${fallbackName}"`);
          created = await zoho.createCustomer({ name: fallbackName, ...customerOpts });
        } else {
          throw err;
        }
      }
