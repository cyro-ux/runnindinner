...
// Zoho invoice-template voor runningdinner.app-facturen. De template bevat
// ons eigen logo, headertekst en footergegevens (VMH B.V., KVK, BTW). Zonder
// deze ID gebruikt Zoho de default VMH BV-template. Overschrijfbaar via env
// zodat accountant het later kan wijzigen zonder code-aanpassing.
const INVOICE_TEMPLATE_ID = process.env.ZOHO_INVOICE_TEMPLATE_ID || '4767620000000614105';
...
async function syncPayment(db, paymentId) {
  ...
  try {
    if (!customerId) {
      const existing = await zoho.findCustomerByEmail(payment.email);
      if (existing?.contact_id) {
        customerId = existing.contact_id;
      } else {
        const created = await zoho.createCustomer({
          name:       payment.company_name || payment.email,
          email:      payment.email,
          country,
          vatId:      payment.vat_id,
          isBusiness: !!payment.is_business,
