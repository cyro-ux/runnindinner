 ...
 *   ZOHO_REGION          — 'eu' (default), 'com', 'in', 'com.au', …
 *
 * De client:
...
async function call(method, path, { body = null, query = {} } = {}) {
  ...
  // 401 → force token refresh, retry one time
  if (resp.status === 401) {
...
/**
 * Zoek customer op e-mailadres. Retourneert null bij geen match.
 */
async function findCustomerByEmail(email) {
  const body = await call('GET', '/books/v3/contacts', { query: { email } });
 ...
 * @param {number} opts.grossCents        totaal incl. BTW (onze interne representatie)
 * @param {number} opts.rate              BTW-percentage (0, 19, 21, ...)
 ...
 * @param {string} opts.taxCode           Zoho tax-code key (zie VatResolver)
 * @param {string} [opts.exemptionReason] indien 0% — wordt als notitie op factuur gezet
 */
async function createInvoice(opts) {
  const {
    customerId, currency, grossCents, rate, description,
    mollie_payment_id, taxCode, exemptionReason,
  } = opts;

  const amount = grossCents / 100;
  const body = {
    ...
    line_items: [{
      name: description,
      rate: amount, // Zoho: rate is unit price here
      quantity: 1,
      tax_id: taxCode,   // moet in Zoho-config gemapt zijn
      item_total: amount,
    }],
