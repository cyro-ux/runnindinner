...
async function createInvoice(opts) {
  ...
  // NB: ignore_auto_number_generation is een QUERY-param in de Zoho API,
  // niet een body-veld (error "nummer komt niet overeen met auto-gen").
  const query = {};
  if (invoiceNumber) {
  ...
  const resp = await call('POST', '/books/v3/invoices', { body, query });
  return resp?.invoice;
}
