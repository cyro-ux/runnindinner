...
/**
 * Zet de actuele reviewscore in het SoftwareApplication-JSON-LD.
 * `rating` = { count, avg } uit de ratings-tabel; zonder goedgekeurde
 * reviews (of bij een leesfout) blijft de statische fallback in de HTML staan.
 */
function applyAggregateRating(html, rating) {
  if (!rating || !(rating.count >= 1) || !(rating.avg > 0)) return html;
  return html
    .replace(/"ratingValue":\s*"[^"]*"/, '"ratingValue": "' + Number(rating.avg).toFixed(1) + '"')
    .replace(/"reviewCount":\s*"[^"]*"/, '"reviewCount": "' + Math.floor(rating.count) + '"');
}

module.exports = { applyI18n, applyCms, pickCmsForLang, applyAggregateRating, escapeHtml };
