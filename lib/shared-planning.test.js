...
test('validatePublishPayload: eigen ganglabel mee (genormaliseerd op 40 tekens)', () => {
  const base = {
    eventName: 'Test', eventDate: '2026-11-14', locale: 'en',
    courses: [
      { course: 'voorgerecht', time: '18:00', endTime: '19:00', label: 'Starter' },
      { course: 'naborrel', time: '21:00', endTime: '23:00', label: '  Quiz' + 'x'.repeat(60) },
      { course: 'hoofdgerecht', time: '19:00', endTime: '20:00' },
    ],
    participants: [{ name: 'Cliff', route: [{ course: 'voorgerecht', address: 'Tafel 1' }] }],
  };
  const v = validatePublishPayload(base);
  assert.strictEqual(v.courses[0].label, 'Starter');
  assert.strictEqual(v.courses[1].label.length, 40, 'label afgekapt op 40');
  assert.strictEqual(v.courses[2].label, undefined, 'geen label = niet aanwezig');
});
