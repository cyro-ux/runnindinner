...
function _parseFrontmatter(text) {
  // Normalise CRLF -> LF so the regex below and the per-line split both
  // work on files saved from Windows editors or checked in through Git
  // with autocrlf. Without this, the trailing \r on each key=value line
  // breaks the $ anchor in the key regex and every field silently fails.
...
/**
 * Related-posts lookup voor internal linking (SEO).
 *
 * Strategie:
 *  1. Keyword-overlap — posts met overlappende keywords scoren hoger.
 *  2. Recency als tiebreaker — nieuwere posts voor elkaar.
 *  3. Locale-filter — alleen posts in dezelfde taal (geen cross-taal linking).
 *  4. Exclude self + drafts.
 *
 * Retourneert top N (default 3) posts met minimaal meta voor rendering.
 */
function getRelated(slug, locale = 'nl', limit = 3) {
  const all  = _getCached().filter(p => !p.draft && p.locale === locale && p.slug !== slug);
  const self = _getCached().find(p => p.slug === slug && p.locale === locale);
  if (!self) return all.slice(0, limit);

  const selfKeywords = String(self.keywords || '')
    .toLowerCase()
    .split(/[,;]\s*/)
    .map(k => k.trim())
    .filter(Boolean);
  ...
  const scored = all.map(p => {
    const kw = String(p.keywords || '').toLowerCase();
    let score = 0;
    for (const k of selfKeywords) if (kw.includes(k)) score++;
    // Date tiebreaker (later date = higher)
    const dateScore = p.date ? new Date(p.date).getTime() / 1e12 : 0;
    return { p, score: score + dateScore };
  });

  return scored.sort((a, b) => b.score - a.score).slice(0, limit).map(s => s.p);
}

// Rendered-HTML cache per "<locale>/<slug>". The markdown-to-HTML parser
// isn't expensive (<5ms per post), but since blog content is mutable only
// via git-deploy we can cache indefinitely and invalidate alongside the
// metadata cache. This removes ~200 KB of allocation per request at scale.
const _renderCache = new Map();

function render(post) {
  const key = `${post.locale}/${post.slug}`;
  const cached = _renderCache.get(key);
  if (cached && cached.ts > _cacheTime) return cached.html;
...
module.exports = { listPublished, listAll, getBySlug, getRelated, render, invalidate, setDraft, mdToHtml };
