...
// fenced code blocks). Kept small — no external deps.
function mdToHtml(md) {
  ...
  // Lists (consecutive - lines become <ul>)
  html = html.replace(/(^|\n)((?:- .+\n?)+)/g, (m, pre, list) => {
  ...
  html = html.split(/\n\n+/).map(block => {
    ...
    if (/^<(h\d|ul|ol|pre|blockquote|div|table|p)\b/i.test(trimmed)) return trimmed;
