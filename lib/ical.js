...
// Interpreteer een datum+tijd als Europe/Amsterdam-wandkloktijd, ongeacht de
// tijdzone van de server. `new Date('YYYY-MM-DDTHH:mm:ss')` zonder suffix
// wordt als LOKALE servertijd geparsed — de productie-VPS draait op UTC,
// waardoor "18:00" stilletjes 20:00 NL-tijd werd. (Gevangen door het
// CI-testgate: de test slaagde alleen op machines in de NL-tijdzone.)
// DST-veilig: de offset wordt per datum bepaald (CET +1 / CEST +2); voor
// dinertijden is de 02:00-overgangsrand niet relevant.
function amsterdamTime(dateStr, timeStr) {
  const utcGuess = new Date(`${dateStr}T${timeStr}Z`);
  const tzPart = new Intl.DateTimeFormat('en-US', {
    timeZone: 'Europe/Amsterdam', timeZoneName: 'shortOffset',
  }).formatToParts(utcGuess).find(p => p.type === 'timeZoneName').value; // bv. "GMT+2"
  const m = tzPart.match(/GMT([+-])(\d+)(?::(\d+))?/);
  const sign = m && m[1] === '-' ? -1 : 1;
  const offMin = m ? sign * (parseInt(m[2], 10) * 60 + parseInt(m[3] || '0', 10)) : 0;
...
function buildEventCalendar(event) {
  const start = event.date ? amsterdamTime(event.date, '18:00:00') : new Date();
  const end   = event.date ? amsterdamTime(event.date, '23:30:00') : new Date();
  const desc = [
    'Running Dinner',
  ...
  return build([{
    uid:         `event-${event.id}@runningdinner.app`,
    start,
    end,
    summary:     event.name,
    description: desc,
    location:    event.location_note || '',
    url:         `https://runningdinner.app/events/${event.id}`,
  }], { calName: event.name });
}
...
function buildParticipantCalendar(event, participant, courses) {
  const dayStart = event.date ? amsterdamTime(event.date, '18:00:00') : new Date();
  const ics = [];
  ...
  for (const c of courses) {
    ...
    ics.push({
      uid:         `course-${event.id}-${participant.id}-${c.name}@runningdinner.app`,
      start,
      end,
...
module.exports = { build, buildEventCalendar, buildParticipantCalendar, amsterdamTime };
