...
const COURSE_KEYS = ['voorborrel', 'voorgerecht', 'hoofdgerecht', 'nagerecht', 'extra1', 'extra2', 'naborrel'];
...
function validatePublishPayload(body) {
  ...
  const courses = body.courses.map(c => {
    ...
    // Eigen ganglabel van de organisator (bv. "Quiz"); server-side
    // ge-escaped bij het renderen, hier alleen normaliseren.
    const label = String(c.label || '').trim().slice(0, 40);
    return label ? { course, time, endTime, label } : { course, time, endTime };
  });
