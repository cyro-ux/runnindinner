...
function renderRevealPage(locale, title, inner, nextRevealMs) {
  ...
  // Auto-reload op het onthulmoment via extern script + data-attribuut:
  // de CSP staat geen inline <script> meer toe (script-src zonder
  // 'unsafe-inline'); /js/reveal-reload.js doet niets zonder het attribuut.
  const reloadScript = nextRevealMs
    ? `<script src="/js/reveal-reload.js" data-reveal-at="${Number(nextRevealMs)}"></script>`
    : '';
  return `<!DOCTYPE html>
<html lang="${locale}">
<head>
