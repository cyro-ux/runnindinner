...
function renderErrorPage(status, locale) {
  const L = {
    ...
    de: {
      title: status === 404 ? 'Seite nicht gefunden' : 'Etwas ist schiefgelaufen',
      heading: status === 404 ? 'Diese Seite existiert nicht' : 'Bei uns ist etwas schiefgelaufen',
      sub: status === 404
        ? 'Der Link ist möglicherweise veraltet oder falsch geschrieben. Nutze den Button unten, um weiterzumachen.'
        : 'Wir haben den Fehler registriert und schauen ihn uns an. Versuche es in ein paar Minuten erneut.',
      cta: '← Zurück zur Startseite',
      blog: 'Oder lies einen Blogartikel',
    },
  };
  // Onbekende taal: val terug op EN (voorheen een kale "Error/Error"-stub)
  // en laat de links dan naar de EN-varianten wijzen i.p.v. /<onbekend>/.
  const known = Object.prototype.hasOwnProperty.call(L, locale);
  const lang = known ? locale : 'en';
  const l = L[lang] || L.nl;
  const homeHref = lang === 'nl' ? '/' : '/' + lang + '/';
  const blogHref = lang === 'nl' ? '/blog' : '/' + lang + '/blog';
