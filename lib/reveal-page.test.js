...
test('renderRevealPage: locale, inhoud en reload-script', () => {
  ...
  assert.ok(html.includes('data-reveal-at='), 'auto-reload-script bij nextRevealMs');
  assert.ok(!renderRevealPage('nl', 'x', 'y', null).includes('reveal-reload'), 'geen reload zonder nextRevealMs');
});
