...
function shareEventInvite({ eventName, eventDate, registerUrl, locale = 'nl' }) {
  ...
  const texts = {
    ...
    de: `🍽️ Running Dinner: *${eventName}* am ${eventDate}\n\nIch organisiere es über runningdinner.app — melde dich hier an:\n\n${url}`,
  };
...
function shareParticipantSchedule({ participantName, phone, personalUrl, locale = 'nl' }) {
  ...
  const texts = {
    ...
    de: `Hallo ${participantName}! 👋

Deine persönliche Route für das Running Dinner findest du hier:
...
function sharePersonalSchedule({ eventName, personalUrl, locale = 'nl' }) {
  ...
  const texts = {
    ...
    de: `Ich mache mit bei *${eventName}* über runningdinner.app 🍽️ Sieh dir meinen Ablauf an: ${url}`,
  };
  return buildShareUrl({ text: texts[locale] || texts.nl });
}
