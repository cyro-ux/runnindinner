...
// ── Mail: zie lib/mailer.js (tranche 10 van de opsplitsing) ─────────────────
const { sendMail, wrapHtml, wrapInvoiceHtml, sendInvoiceMail, formatEur } = require('./lib/mailer');
// ── Helpers ───────────────────────────────────────────────────────────────────
...
function checkReferralReward(referrerId) {
  ...
  for (let i = rewardsEarned; i < expectedRewards; i++) {
    ...
    const subj = { nl: '🎉 Gratis jaar verdiend via referrals',
                   ...
                   es: '🎉 Año gratis ganado gracias a referidos',
                   de: '🎉 Gratisjahr durch Empfehlungen verdient' }[lang] || 'Free year earned';
    // Let op: fallback op NL — een onbekende taal gaf hier voorheen een
    // undefined-body (Duitse referrers kregen een lege mail).
    const body = ({
      ...
      de: `<p>Hallo,</p><p>Tolle Neuigkeiten! Drei Personen haben sich über deinen Einladungslink angemeldet — als Dankeschön hast du <strong>1 Gratisjahr</strong> erhalten. Dein Abonnement läuft jetzt bis zum <strong>${new Date(newUntil).toLocaleDateString('de-DE')}</strong>.</p><p>Danke, dass du den Running Dinner Planner weiterempfiehlst!</p>`,
    })[lang] || `<p>Hi,</p><p>You've earned 1 free year. Your subscription now runs until <strong>${new Date(newUntil).toLocaleDateString('en-GB')}</strong>.</p>`;
...
// ── Start ─────────────────────────────────────────────────────────────────────
// De app wordt geëxporteerd zodat (CI-)tests hem kunnen booten zonder poort;
// listen() draait alleen bij direct starten (node server.js / PM2).
module.exports = app;
